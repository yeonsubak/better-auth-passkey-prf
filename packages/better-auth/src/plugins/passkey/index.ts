...
export const passkey = (options?: PasskeyOptions) => {
  ...
  const maxAgeInSeconds = Math.floor((expirationTime.getTime() - currentTime.getTime()) / 1000);
    ...
    endpoints: {
      ...
      verifyPasskeyRegistration: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const { expectedChallenge, userData } = JSON.parse(data.value) as WebAuthnChallengeValue;
          ...
          try {
            ...
            const newPasskeyRes = await ctx.context.adapter.create<Omit<Passkey, 'id'>, Passkey>({
              model: 'passkey',
              data: newPasskey,
            });
      ...
      verifyPasskeyAuthentication: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const { expectedChallenge } = JSON.parse(data.value) as WebAuthnChallengeValue;
          ...
          try {
            ...
            const s = await ctx.context.internalAdapter.createSession(passkey.userId, ctx);
            if (!s) {
              throw new APIError('INTERNAL_SERVER_ERROR', {
                  ...
                  content: {
                    'application/json': {
                      schema: {
                        ...
                        items: {
                          $ref: '#/components/schemas/Passkey',
                          required: ['id', 'userId', 'publicKey', 'createdAt', 'updatedAt'],
                        },
